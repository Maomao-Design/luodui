...
<div class="subnav">
	...
	<div class="table-list">
		...
		<tr>
			...
			<td align="left">
			<?php if ($this->ci->is_auth('module/config')) { ?><a style="color:#00F" href="<?php echo dr_url('module/config',array('id'=>$t['id'], 'all' => 1)); ?>"><?php echo lang('061'); ?></a>&nbsp;&nbsp;<?php }  if (!$t['mydb']) {  if ($this->ci->is_auth('mform/index')) { ?><a href="<?php echo dr_url('mform/index',array('dir'=>$dir)); ?>"><?php echo lang('html-663'); ?></a>&nbsp;&nbsp;<?php }  if ($this->ci->is_auth('module/config')) { ?><a href="javascript:;" onclick="dr_copy_module('<?php echo dr_url('module/copy',array('dir'=>$dir)); ?>')"><?php echo lang('html-513'); ?></a>&nbsp;&nbsp;<?php }  if ($this->ci->is_auth('module/config')) { ?><a style="color: #090" href="javascript:;" onclick="dr_module_export('<?php echo dr_url('module/export',array('dir'=>$dir)); ?>')"><?php echo lang('html-516'); ?></a>&nbsp;&nbsp;<?php }  if ($this->ci->is_auth('admin/field/index')) { ?><a href="<?php echo $duri->uri2url('admin/field/index/rname/module/rid/'.$t['id']); ?>"><?php echo lang('html-590'); ?></a>&nbsp;&nbsp;<?php }  if ($t['extend'] && $this->ci->is_auth('admin/field/index')) { ?><a href="<?php echo $duri->uri2url('admin/field/index/rname/extend/rid/'.$t['id']); ?>"><?php echo lang('html-591'); ?></a>&nbsp;&nbsp;<?php }  if ($this->ci->is_auth('module/uninstall')) { ?><a href="javascript:;" onClick="return dr_confirm_url('<?php echo lang('html-608'); ?>','<?php echo dr_url('module/clear',array('dir'=>$dir)); ?>');"><?php echo lang('html-607'); ?></a>&nbsp;&nbsp;<?php }  }  if ($this->ci->is_auth('module/uninstall')) { ?><a href="javascript:;" onClick="return dr_confirm_url('<?php echo lang('html-170'); ?>','<?php echo dr_url('module/uninstall',array('id'=>$t['id'])); ?>');"><?php echo lang('html-164'); ?></a>&nbsp;&nbsp;<?php }  if (!$t['site'][SITE_ID]['use']) { ?><font color=red>[<?php echo lang('html-168'); ?>]</font>&nbsp;&nbsp;<?php } else { ?><a href="<?php echo $t['url']; ?>" target="_blank"><?php echo lang('go'); ?></a><?php }  if ($this->ci->is_auth('module/update')) { ?>&nbsp;&nbsp;<a class="dr_update_<?php echo $dir; ?>" style="display:none; color:#090" href="<?php echo dr_url('module/update',array('id'=>$dir)); ?>"><?php echo lang('html-167'); ?></a>&nbsp;&nbsp;<?php }  if ($t['key']) { ?><span class="dr_check_<?php echo $dir; ?>">正在云端检查版本...</span><?php } ?>
            ...
            <script type="text/javascript">
                <?php if (is_array($t['site'])) { $count=count($t['site']);foreach ($t['site'] as $s) {  if ($s['domain']) { ?>
                    $.get("<?php echo dr_url('home/domain', array('domain' => $s['domain'])); ?>", function(data){
                        if (data) {
                            $("#dr_domain_<?php echo $dir; ?>").html("<a href='<?php echo dr_url('module/config',array('id'=>$t['id'])); ?>' style='color:red;'>域名解析异常</a>");
                        }
                    });
                    <?php }  } } ?>
            </script>
