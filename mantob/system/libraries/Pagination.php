...
class CI_Pagination {
	...
	/**
	 * Generate the pagination links (v3)
	 * total_rows 	总数量
	 * per_page 	每页数量
	 * cur_page 	当前页
	 * num_pages    分页数
	 * @return	string
	 */
	public function mantou_mao_links2()
	{
		// If our item count or per-page total is zero there is no need to continue.
		if ($this->total_rows === 0 OR $this->per_page === 0)
		{
		...
		// 	 $output .= '<span>...</span>';
       	// }
		// if(($new_start > 1 && $cur_page < 4) || $cur_page == 1){
		// 	$new_end++;
		// }
		// if($cur_page > $num_pages-4 && $cur_page >= $num_pages){
		// 	$new_start--;
		// }
    ...
    //    }
       ...
       $start = $cur_page -2;
	   $end = $cur_page+2;
       if((start > 1 && $cur_page < 4)||$cur_page == 1){
         $end++;
       }
       if($cur_page > $num_pages-4 && $cur_page >= $num_pages){
         $start--;
    ...
    //    }
			...
			// Write the digit links
			// for ($loop = $new_start; $loop <= $new_end; $loop++)
			// {
			// 	$i = ($this->use_page_numbers) ? $loop : ($loop * $this->per_page) - $this->per_page;
